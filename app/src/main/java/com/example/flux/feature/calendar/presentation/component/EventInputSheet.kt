...
fun EventInputSheet(
    ...
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(
            ...
            OutlinedTextField(value = title, onValueChange = { title = it }, label = { Text("事件标题") }, modifier = Modifier.fillMaxWidth(), singleLine = true)
            OutlinedTextField(value = description, onValueChange = { description = it }, label = { Text("描述") }, modifier = Modifier.fillMaxWidth(), maxLines = 3)
            ...
            if (recurrenceRule != "none") {
                ...
                if (!isRecurrenceUntilValid) {
                    Text("重复截止日期应为 YYYY-MM-DD", color = MaterialTheme.colorScheme.error, style = MaterialTheme.typography.bodySmall)
                }
            }
            if (!isTimeValid) {
                Text("时间应为 HH:mm", color = MaterialTheme.colorScheme.error, style = MaterialTheme.typography.bodySmall)
            }
